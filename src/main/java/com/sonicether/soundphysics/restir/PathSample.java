 ...
 * {@code seedDir} plus {@code bounces} deterministically re-traces the
 * identical chain from the owning cell's ray origin, which is how
 ...
 * in enclosed spaces. {@code legTransmission}/{@code legTransmissionLow} are
 * the latest measured final-leg (lastHit → listener) transmissions per band,
 * refreshed by the batch; the final leg is traced fresh, so one sample serves
 * a moving listener. {@code chainReflectivity} (average reflectivity along
 * the chain) feeds the inherited distance→delay mapping. Immutable; updates
 * swap in a copy (single-writer copy-on-write discipline in {@link Cell}).
 */
public record PathSample(
		...
		float legTransmissionLow,
		long lastValidatedTick) {

	public PathSample withLegTransmission(final float high, final float low) {
		return new PathSample(seedDirX, seedDirY, seedDirZ, bounces, lastHitX, lastHitY, lastHitZ,
				totalDistance, energy, chainReflectivity, high, low, lastValidatedTick);
	}
	...
	public PathSample revalidated(final float hitX, final float hitY, final float hitZ,
		...
		return new PathSample(seedDirX, seedDirY, seedDirZ, bounces, hitX, hitY, hitZ,
				distance, newEnergy, newChainReflectivity, legTransmission, legTransmissionLow, tick);
	}
