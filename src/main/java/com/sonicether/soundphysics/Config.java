...
public class Config {
	...
	public boolean syncConfig() {
		...
		snowAirAbsorptionFactor = this.forgeConfig.getFloat("Max Snow Air Absorption Factor", categoryGeneral, 5.0f,
				0.0f, 10.0f,
				"The maximum air absorption factor when it's snowing on the listener. The real factor scales with the snow's intensity. Set to 1 or lower to disable.");
		underwaterFilter = this.forgeConfig.getFloat("Underwater Filter", categoryGeneral, 0.8f, 0.0f, 1.0f,
				"How much sound is filtered when the listener is underwater. 0.0 means no filter. 1.0 means fully filtered.");
